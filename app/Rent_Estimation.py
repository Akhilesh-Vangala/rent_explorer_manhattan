...
st.set_page_config(
    page_title="Rent Estimation Tool",
    page_icon=None,
    layout="wide",
...
AFFORDABILITY_CRITERIA = {
    -15: ["Major Affordable", "#10b981"],
    -5: ["Affordable", "#34d399"],
    5: ["Fairly Priced", "#fbbf24"],
    15: ["Premium", "#f97316"],
}
    ...
    with st.spinner("Calculating rent prediction..."):
        ...
        if neighborhood_data.empty:
            ...
            st.session_state.stats = {
                'avg_rent': float(df_preprocessed['rent'].mean()),
                'median_rent': float(df_preprocessed['rent'].median()),
                'rent_per_sqft': float(df_preprocessed['rent_per_sqft'].mean())
            }
        else:
            avg_rent_per_sqft = float(neighborhood_data['rent_per_sqft'].mean())
            avg_log_rent = float(neighborhood_data['log_rent'].mean())
            st.session_state.stats = {
                'avg_rent': float(neighborhood_data['rent'].mean()),
                'median_rent': float(neighborhood_data['rent'].median()),
                'rent_per_sqft': float(neighborhood_data['rent_per_sqft'].mean())
            }
...
if 'prediction' in st.session_state:
    ...
    for affordability_threshold, affordability_label in AFFORDABILITY_CRITERIA.items():
        if diff_avg <= affordability_threshold:
            affordability_label = affordability_label[0]
            affordability_color = affordability_label[1]
            break
    else:
