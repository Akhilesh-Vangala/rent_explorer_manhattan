...
for n in neighborhoods:
    ...
    if len(n_data) > 0:
        ...
        neighborhood_stats.append({
            'Neighborhood': n,
            'Average Rent': rent_values.mean(),
            'Median Rent': np.median(rent_values),
            'Min Rent': rent_values.min(),
            'Max Rent': rent_values.max(),
            'Pct25': np.percentile(rent_values, 25),
            'Pct75': np.percentile(rent_values, 75),
            'Count': len(n_data)
        })
...
# Rename column to match visualization function expectation
