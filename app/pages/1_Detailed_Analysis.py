...
from src.visualization import (
    plot_rent_comparison,
    plot_size_vs_rent,
    plot_rent_distribution,
    plot_neighborhood_comparison,
    plot_amenity_impact,
    plot_bedroom_rent_distribution,
    plot_feature_values
)
...
for n in neighborhoods:
    try:
        ...
        if len(n_data) > 0:
            all_neighborhood_avgs.append({
                'neighborhood': n,
                'avg_rent': n_data['rent'].mean()
            })
    except ValueError:
        # Skip neighborhoods not found in data
        continue
...
features = {
    'Size (sqft)': inputs['size_sqft'],
    'Bedrooms': inputs['bedrooms'],
    'Bathrooms': inputs['bathrooms'],
    'Floor': inputs['floor'],
    'Building Age': inputs['building_age_yrs'],
    'Minutes to Subway': inputs['min_to_subway']
}
