"""
Model Retraining Script - SmartRent Manhattan
...
Author: SmartRent Manhattan Team
...
from modeling import train_test_split_df, train_model, evaluate_model, get_cv_metrics, print_model_metrics

def main():
    """Main function to retrain and evaluate the model."""
    ...
    lr_metrics = {
        'train': evaluate_model(lr_model, X_train, y_train),
        'val': evaluate_model(lr_model, X_val, y_val),
        'test': evaluate_model(lr_model, X_test, y_test),
        'time': lr_time
    }
    ...
    rf_metrics = {
        ...
        'test': evaluate_model(rf_model, X_test, y_test),
        'time': rf_time
    }
    ...
    xgb_metrics = {
        'train': evaluate_model(xgb_model, X_train, y_train),
        'val': evaluate_model(xgb_model, X_val, y_val),
        'test': evaluate_model(xgb_model, X_test, y_test),
        'time': xgb_time
    }
    ...
    # os.makedirs(Config.MODEL_METRICS_FILE, exist_ok=True)
    ...
    # os.makedirs(Config.CV_METRICS_FILE, exist_ok=True)
